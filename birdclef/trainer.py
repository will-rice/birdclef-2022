...
class Trainer:
    ...
    def __init__(self, config, model: tf.keras.Model, dataset: Dataset, log_dir: Path):
        ...
        self.loss_fn = tfa.losses.SigmoidFocalCrossEntropy(from_logits=True)
        self.metric_fn = tf.keras.metrics.BinaryAccuracy(threshold=0.0)
        ...
        self.optimizer = tfa.optimizers.AdamW(learning_rate=3e-4, weight_decay=1e-6)
